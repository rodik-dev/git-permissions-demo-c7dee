...
export const page_model_2: Model = {
    ...
    permissions: (options) => {
        ...
        if (options.userContext.sso?.attributes?.groups?.includes('data_model_2_editor_role')) {
            return { canView: true, canEdit: true, canPublish: false };
        }

        // allow a specific role to edit and publish
        if (options.userContext.sso?.attributes?.groups?.includes('data_model_2_publisher_role')) {
